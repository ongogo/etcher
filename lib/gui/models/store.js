...
const storeReducer = (state = DEFAULT_STATE, action) => {
  switch (action.type) {
    ...
    case ACTIONS.SET_FLASH_STATE: {
      ...
      const HUNDRED = 100;
      if (action.data.percentage > HUNDRED) {
        throw errors.createError(`Excessive state percentage: ${action.data.percentage}`);
      }

      const ZERO = 0;
      if (action.data.percentage < ZERO) {
        throw errors.createError(`Deficient state percentage: ${action.data.percentage}`);
      }
