...
module.exports = function(
  ...
  /**
   * @summary Get the of drives and their compatibility status objects
   * @function
   * @public
   *
   * @description
  ...
  this.getDrivesAndStatuses = memoizeImmutableListReference(() => {
    ...
    for (const drive of this.drives.getDrives()) {
      const statuses = this.constraints.getDriveImageCompatibilityStatuses(drive, this.state.getImage());
      list.push({
        drive,
        statuses
      });
    ...
    return _.every(_.zipWith(newList, oldList, (newStatus, oldStatus) => {

      // Remove Angular hashes that are always unique if the object is new
      const newAttrs = _.omit(newStatus, [ '$$hashKey' ]);
      const oldAttrs = _.omit(oldStatus, [ '$$hashKey' ]);
      newAttrs.statuses = _.map(newAttrs.statuses, (status) => {
        return _.omit(status, [ '$$hashKey' ]);
      });
      oldAttrs.statuses = _.map(newAttrs.statuses, (status) => {
        return _.omit(status, [ '$$hashKey' ]);
      });
      return _.isEqual(newAttrs, oldAttrs);
    }));
  });
